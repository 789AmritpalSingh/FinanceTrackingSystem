...
const PersonalExpenses = () => {
  ...
  const handleAddExpense = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json(); // Parse the response to get the new expense ID
        // Dispatch the new expense to the Redux store with the returned ID
        dispatch(addExpense({ ...newExpense, id: data.expense_id }));
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/update_expense/${expenseToUpdate.id}`,
        {
          ...
          body: JSON.stringify(expenseToUpdate), // Send the updated data to the backend
        }
      );
  
      if (response.ok) {
        dispatch(updateExpense(expenseToUpdate)); // Dispatch the updated expense to Redux
        setSnackbarSeverity("success");
        setSnackbarMessage("Expense updated successfully.");
        setSnackbarOpen(true);
        handleUpdateModalClose(); // Close the modal after updating
      } else {
        setSnackbarSeverity("error");
      ...
      setSnackbarSeverity("error");
      setSnackbarMessage(`Error updating expense: ${error}`);
      setSnackbarOpen(true);
    }
  };
